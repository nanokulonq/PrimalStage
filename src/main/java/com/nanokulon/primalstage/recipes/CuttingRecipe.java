...
public class CuttingRecipe implements Recipe<Inventory> {
    ...
    public static final Codec<CuttingRecipe> CODEC = RecordCodecBuilder.create(instance -> instance.group(
            Registries.ITEM.getCodec().fieldOf("input").forGetter(CuttingRecipe::getInput),
            Codec.INT.fieldOf("totalhits").forGetter(CuttingRecipe::getTotalHits),
            Registries.ITEM.getCodec().fieldOf("result").forGetter(CuttingRecipe::getOutput)
    ...
    public Identifier getId() {
        return new Identifier(PrimalStage.MOD_ID, output.toString() + "_from_cutting");
    }
