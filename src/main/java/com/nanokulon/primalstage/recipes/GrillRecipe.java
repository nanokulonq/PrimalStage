...
public class GrillRecipe implements Recipe<Inventory> {

    private final Item input;
    private final int cookingTime;
    private final Item output;
    public static final Codec<GrillRecipe> CODEC = RecordCodecBuilder.create(instance -> instance.group(
            Registries.ITEM.getCodec().fieldOf("input").forGetter(GrillRecipe::getInput),
            Codec.INT.fieldOf("cookingtime").forGetter(GrillRecipe::getCookingTime),
            Registries.ITEM.getCodec().fieldOf("result").forGetter(GrillRecipe::getOutput)
    ...
    public Identifier getId() {
        return new Identifier(PrimalStage.MOD_ID, output.toString() + input.toString() + "_from_grill");
    }
