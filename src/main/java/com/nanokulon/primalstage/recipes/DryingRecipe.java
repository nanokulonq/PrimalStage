...
public class DryingRecipe implements Recipe<Inventory> {
    ...
    public static final Codec<DryingRecipe> CODEC = RecordCodecBuilder.create(instance -> instance.group(
            Registries.ITEM.getCodec().fieldOf("input").forGetter(DryingRecipe::getInput),
            Codec.INT.fieldOf("cookingtime").forGetter(DryingRecipe::getCookingTime),
            Registries.ITEM.getCodec().fieldOf("result").forGetter(DryingRecipe::getOutput)
    ...
    public Identifier getId() {
        return new Identifier(PrimalStage.MOD_ID, output.toString() + "_from_drying");
    }
